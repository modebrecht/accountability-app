...
export function isCompletedWithinDays(task, days, today = new Date()) {
...
export function isCompletedOlderThan(task, days, today = new Date()) {
    if (!task.completed_at)
        return false;
    const completedAt = new Date(task.completed_at);
    const diff = today.getTime() - completedAt.getTime();
...
export function splitTasks(tasks, today = new Date()) {
...
export function pickTaskForNotification(tasks, today = new Date()) {
