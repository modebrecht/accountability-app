...
function createTasksFetchBuilder(data, error = null) {
    return {
        select: vi.fn().mockReturnThis(),
        order: vi.fn().mockResolvedValue({ data, error })
    };
}
function createCompletionsFetchBuilder(data, error = null) {
    return {
        select: vi.fn().mockReturnThis(),
...
function createUpdateBuilder(data, error = null) {
    return {
...
function createDeleteBuilder(error = null) {
    return {
...
describe('useTasks', () => {
    ...
    it('surfaces query errors when the initial fetch fails', async () => {
        const error = { message: 'Unable to fetch tasks' };
